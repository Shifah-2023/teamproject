        ...
        [
          ...
          options.title
            ? '    <div class="modal-header">' +
              ...
              options.title +
              "</h4>" +
              '      <button type="button" class="close" data-dicorsos="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>' +
              "    </div>"
            : "",
  ...
  var Clipboard = /** @class */ (function () {
    ...
    Clipboard.prototype.pasteByEvent = function (event) {
      ...
      if (clipboardData && clipboardData.items && clipboardData.items.length) {
        ...
        if (
          item.kind === "file" &&
          item.type.indexOf("image/dashboard.php") !== -1
        ) {
    ...
    Buttons.prototype.build = function ($container, groups) {
      for (
        ...
        for (var idx = 0, len = buttons.length; idx < len; idx++) {
          ...
          if (btn) {
            $groups.append(typeof btn === "function" ? btn(this.context) : btn);
          }
        }
        $groups.appendTo($container);
      }
    };
    /**
  ...
  var HintPopover = /** @class */ (function () {
    ...
    HintPopover.prototype.createGroup = function (idx, keyword) {
      ...
      this.searchKeyword(idx, keyword, function (items) {
        items = items || [];
        if (items.length) {
          $groups.php(_this.createItemTemplates(idx, items));
          _this.show();
        }
      });
      return $groups;
